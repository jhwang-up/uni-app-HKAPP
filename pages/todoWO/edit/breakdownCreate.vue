		...
		<view class="info-box">
			...
			<view class="info">
				...
				<button class="wxr-btn" type="default" @click="showTree"
					:disabled="myData.wxrlx.length===0">{{myData.wxrlx.length>0?(myData.gzwxr||'点击选择维修人'):'请先选择维修人类型'}}</button>
			</view>
			<view class="info">
...
<script>
	// 已知的bug：1. 故障维修人不能包涵父级别节点名称～！！！～～
	// 故障工单的几个状态分别做啥处理，问下王兆驰 1h
	...
	import {
		...
		getBreakdownContent, // 获取故障工单信息  当前页面可能用不到
	...
	export default {
		...
		onLoad(options) {
			this.myData = {
				...
				gzgdwcsj: 'YYYY-MM-DD',
				gzfxsj: 'YYYY-MM-DD',
				wjuuid: uuidv4(), // 此页面是新增页面，则每次页面加载后需要生成一次新的wjuuid
			};
		...
		methods: {
			...
			*/
			...
			imgDelete(list, eq, deleteUrl) {
				...
				// 如果没有 ‘&mid=’  说明是临时图片，不需要调用接口删除  但是临时的图片已经绑定了啊 所以在上传完后就得重新获取图片链接
			...
			// 4fa83413-a143-4357-a035-dd2cc2cf4b1d
			// 先把文件渲染出来  10min
			// 删除文件  
			deleteFile(item) {
				// 调用delete的接口即可，
			...
			// 渲染日期插件
			initDate() {
				this.myData.gzfxsj = this.myData.gzfxsj.length > 0 ? this.myData.gzfxsj : 'YYYY-MM-DD';
				this.myData.gzgdwcsj = this.myData.gzgdwcsj.length > 0 ? this.myData.gzgdwcsj : 'YYYY-MM-DD';
			},
			// 查看省略号文字的气泡弹窗
			taptext(e, data) {
				this.x = e.touches[0].clientX
