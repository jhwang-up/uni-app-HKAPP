<template>
					...
					<view class="inner">
						<view class="info-box">
							...
							<view class="info info-no-line">
								...
								<!-- 此处是有点击事件 -->
								<button type="default" size="mini" @click="showTree">{{ysr||'点击选择审核人'}}</button>
							</view>
						</view>
    ...
    display: flex;" ref='uploadImg' :maxCount="3" :mode="imgList" @chooseFile='chooseImg' @imgDelete='imgDelete'
		...
		<uni-popup ref="treepopup">
			<popup-scroll
				style="max-height: 68vh;min-height:100px; width: 700rpx; background-color: #fff;border-radius: 8px;">
				<view style="height: 100%;">
					<ly-tree ref="tree" :tree-data="treeData" :ready="ready" node-key="id" show-checkbox
						:defaultExpandAll="defaultExpandAll" @check="handleCheck">
					</ly-tree>
				</view>
			</popup-scroll>
		</uni-popup>
...
<script>
	...
	import {
		...
		saveProduceWorkContent,
		queryAttachementByUUID,
		submitProduceWorkContent,
		reallySaveFile,
		getAttachmentByMid,
		getProduceWorkContentTree, // 生产系统工单审核人员树数据
	...
	export default {
		...
		methods: {
			...
			// 展示树形选择弹窗Î
			showTree() {
				getProduceWorkContentTree().then(res => {
					...
					this.$nextTick(function() {
						this.setCheckedKeys(this.ysrIdArray);
					})
				}).catch(err => {
					uni.showToast({
						title: `异常，请稍后重试`,
			...
			handleCheck(obj) {
				...
				this.ysr = tempLabel.toString();
				this.ysrgh = tempId.toString();
				// 保存选中的节点key数组，用于渲染选中
				// this.myData.gzwxrIdArray = tempId;
				this.ysrIdArray = tempId;

				// console.log('handleCheck===', JSON.stringify(this.ysrIdArray));
				// console.log('handleCheck===', JSON.stringify(this.ysrgh));
				// console.log('=========================================');
			},
			// ================================= 以下 可选控件 ==================================
			radioChange(evt) {
