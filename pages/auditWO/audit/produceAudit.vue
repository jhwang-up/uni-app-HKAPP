<template>
	<view class="produceEdit">
					...
					<view class="inner">
						<view class="info-box">
							...
							<view class="info info-textarea info-no-line">
								...
								<textarea class="textarea" v-model="ysyj" auto-height="true" placeholder="请输入审核意见" />
							</view>
						</view>
			...
			<view class="inner">
				<view class="info-box" :class="{'info-no-bottom':index===tableData.length-1}">
					...
					<view v-if="item.sfdwzb==='是'" class="info info-textarea">
						...
						<textarea class="textarea" v-model="item.sjdz" disabled='true' auto-height="true" />
					</view>
		...
		<view class="bottom-box">
			<view class="btn-box">
				<button @click="ok()" size="mini" class="btn" type="primary">确认</button>
				<button @click="reject()" size="mini" class="btn" type="default">退回</button>
			</view>
		</view>
...
<script>
	...
	import {
		...
		rejectOrPassProduceWorkContent, //生产系统工单审核退回接口
	...
	export default {
		...
		onLoad(options) {
			...
			this.wcqk = options.wcqk === 'undefined' ? '' : options.wcqk;
			this.getData();
		...
		methods: {
			...
			deal(data) {
				...
				rejectOrPassProduceWorkContent(data).then(res => {
			...
			// 优化table样式 然后引入vuex 1.5h 然后封装附件上传1.5h 再自测试30min
			// 需要引入新版本插件 不对放置位置有要求
			// 如果是第一次上传，需要uuid，那么得传入uuid，
			// 下载：点击下载，完全组件内部逻辑
			// 上传：也是接口什么的也在组件内部，
			// 需要传入的propos是 table的数据，
			// 下载附件
			downloadFile(data) {
				// 只要拿到地址即可吧？？ 
