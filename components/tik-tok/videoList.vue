			...
			<swiper class="swiper" :vertical="true" @change="change" @touchstart="touchStart" @touchend="touchEnd">
				<swiper-item v-for="(item,index) of videos" :key="item.id">
					<view class="swiper-item" style="color:#000000">
						<video-player @changeClick="changeClick" ref="player" :video="item" :index="index">
						</video-player>
					</view>
					<view class="left-box">
	...
	export default {
		...
		watch: {
			list() {
				...
				console.log("videoList==" + JSON.stringify(this.videos));
			}
		},
		mounted() {
			this.videos = this.list;
			console.log("videoList==" + JSON.stringify(this.list));
		},
		methods: {
			changeClick() {
				//点赞，调用listRight方法
