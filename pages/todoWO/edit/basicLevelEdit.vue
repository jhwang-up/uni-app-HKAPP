<template>
	<view class="electromechanical">
		...
		<view class="info-box">
			...
			<view class="info">
				<text class="left">排查时间:</text>
				<text class="right">{{myData.pcsj}}</text>
			</view>
			<view class="info">
...
<script>
	/*
		5. 搞一下查看详情页面  两个列表跳转的都是一个页面，然后区别就是操作按钮多了一个延期
		6. 已执行页面 2h 
		
		已知的bug：
		1. 触发onshow时 zj 咋就为空了？？
	*/
	import {
		v4 as uuidv4
	} from 'uuid';
	...
	import {
		...
		getBaseElectromechanicalContent, // 机电基层隐患工单查询
		getElectromechanicalBdzTree, // 获取机电班队长_选择树数据
		getElectromechanicalZgrTree, // 获取机电整改人_选择树数据
		releaseBaseElectromechanical, // 机电基层隐患工单科区长发布
		submitBaseElectromechanical, // 机电基层隐患班组长整改提交完成 
	} from '../../../common/api.js';
	export default {
		components: {
			LyTree,
		...
		methods: {
			...
			initElectromechanicalData() {
				...
				getBaseElectromechanicalContent({
					zj: this.currentZj
				}).then(res => {
			...
			showTree() {
				if (this.currentStatus === '办理') {
					getElectromechanicalBdzTree({
						maindeptid: this.myData.yhbm
					}).then(res => {
